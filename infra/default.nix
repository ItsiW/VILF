{inputs, ...}: {
      ...
      resource = {
        ...
        google_sql_database_instance.main = {
          ...
          settings = {
            ...
            ip_configuration.authorized_networks = lib.attrsets.mapAttrsToList lib.nameValuePair {
  ...
  in {
    canivete.opentofu.plugins = ["opentofu/google" "opentofu/null"];
    packages.opentofu = config.canivete.opentofu.finalPackage;
    packages.terranix-climax = inputs.terranix.lib.terranixConfiguration {
      inherit pkgs;
      extraArgs = {inherit inputs inputs';};
      modules = [
        config.canivete.opentofu.terranixModule
        ./server.nix
        module
      ];
    };
    apps.tofu = {
      ...
      program = pkgs.writeShellApplication {
        name = "tofu";
        runtimeEnv.config = self'.packages.terranix;
        runtimeInputs = with pkgs; [bash coreutils git self'.packages.opentofu];
    ...
    apps.tofux = {
      ...
      program = pkgs.writeShellApplication {
        name = "tofux";
        runtimeEnv.config = self'.packages.terranix-climax;
        runtimeInputs = with pkgs; [bash coreutils git self'.packages.opentofu];
        text = builtins.readFile ./tofu.sh;
      };
    };
    devShells.infra = pkgs.mkShell {
      packages = with pkgs; [terranix self'.packages.opentofu];
    };
  };
}
